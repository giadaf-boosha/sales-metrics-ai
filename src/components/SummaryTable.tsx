  ...
  const channelSummary = React.useMemo(() => {
    ...
    const summary = data.reduce((acc, curr) => {
      ...
      if (curr.contractsClosed && curr.status === 'Cliente') {
        ...
        if (curr.contractsClosed && curr.meetingCompleted) {
          const closingDate = new Date(curr.contractsClosed);
          const meetingDate = new Date(curr.meetingCompleted);
          const salesCycleDays = Math.floor((closingDate.getTime() - meetingDate.getTime()) / (1000 * 60 * 60 * 24));
    ...
    return Object.values(summary).map(channel => {
      ...
      const avgSalesCycle = channel.totalClosedWonOpps > 0 
        ? channel.totalSalesCycleDays / channel.totalClosedWonOpps 
        : 0;
      const acv = channel.totalClosedWonOpps > 0 
      ...
      return {
        ...
        winRate: winRate * 100,
        pipelineVelocity,
        pipelineContribution: totalPipelineValue > 0 
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right">Total Closed Lost Opps.</TableHead>
              <TableHead className="text-right">Total Closed Won Opps.</TableHead>
              <TableHead className="text-right">Total Closed Won Revenue</TableHead>
              <TableHead className="text-right">ACV</TableHead>
              <TableHead className="text-right">Closed Won Avg. Sales Cycle</TableHead>
              <TableHead className="text-right">Win-Rate</TableHead>
              <TableHead className="text-right">Pipeline Velocity</TableHead>
              <TableHead className="text-right">% of pipeline contribution</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {channelSummary.map((row) => (
              <TableRow key={row.source}>
                ...
                <TableCell className="text-right">{row.totalOppsCreated}</TableCell>
                <TableCell className="text-right">{row.totalClosedLostOpps}</TableCell>
                <TableCell className="text-right">{row.totalClosedWonOpps}</TableCell>
                <TableCell className="text-right">€{row.totalClosedWonRevenue.toLocaleString('it-IT', { minimumFractionDigits: 2 })}</TableCell>
                <TableCell className="text-right">€{row.acv.toLocaleString('it-IT', { minimumFractionDigits: 2 })}</TableCell>
                <TableCell className="text-right">{Math.round(row.closedWonAvgSalesCycle)} giorni</TableCell>
                ...
                <TableCell className="text-right">{row.pipelineContribution.toFixed(2)}%</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
